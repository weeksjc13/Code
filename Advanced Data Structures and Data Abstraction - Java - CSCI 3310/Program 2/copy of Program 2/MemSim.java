...
public class MemSim 
{
  ...
  //in the programs usage and updates the free page list
  public static void shrink(int id, int shrinkBytes, List<PageUsage> freePageList, ProgInfo[] programsArray)
  {
    int extraBytes = programsArray[id].bytes % PAGE_SIZE;				//left over bytes from last page used by the program
    if(extraBytes > 0)
    {
      extraBytes = PAGE_SIZE - extraBytes;
    }
    int currentPages = programsArray[id].bytes / PAGE_SIZE;				//pages used by the program
    if(programsArray[id].bytes % PAGE_SIZE > 0)
    {
      currentPages++;
    }
    int currentBytes = (currentPages * PAGE_SIZE) - extraBytes;				//number of bytes currently used by a program
    int newSize = (currentPages * PAGE_SIZE) - extraBytes - shrinkBytes;		//the bytes to taken up by the program after shrink
    int bytesFreed = currentBytes - newSize; 						//bytes to be freed by shrink
    int extraPages = currentPages - (newSize / PAGE_SIZE);				//pages freed by shrink
    ...
    else if(programsArray[id].bytes <= shrinkBytes)
    {
      System.out.printf("ERROR on shrink command: insufficient allocation for Program %d.\n\n", id);
    }
    else
    {
      if(programsArray[id].prog_usage.size() >= 1)
      {
        int start = programsArray[id].prog_usage.get(1).getStart();	//start of prog usage
        int end = programsArray[id].prog_usage.get(1).getEnd();		//end of prog usage

        for(int index = programsArray[id].prog_usage.size(); bytesFreed > 0; index--)
        {
          int lastBytes = ((end - start) + 1) * PAGE_SIZE;  //bytes held by prog usage at front of list
          if(lastBytes >= bytesFreed)	//enough space to hold bytes
          {
            int pagesFreed = bytesFreed / PAGE_SIZE;	//number of pages to be freed
            
            if(lastBytes == bytesFreed)
            {
              programsArray[id].prog_usage.remove(1);
            }
            else
            {
              programsArray[id].prog_usage.get(1).setStart(programsArray[id].prog_usage.get(1).getEnd() - pagesFreed); //new end value of page usage
            }
            PageUsage newFreePage = new PageUsage(programsArray[id].prog_usage.get(1).getStart() + pagesFreed, programsArray[id].prog_usage.get(1).getEnd()); // new free page to be added
            freePageList.add(1, newFreePage);
            programsArray[id].bytes = newSize;
            break;
          }
          else
          {
            bytesFreed = bytesFreed - lastBytes;
            freePageList.add(1, programsArray[id].prog_usage.get(1));
            programsArray[id].prog_usage.remove(1);
            start = programsArray[id].prog_usage.get(1).getStart();	//the last prog usage's beginning
            end = programsArray[id].prog_usage.get(1).getEnd();	
          }
        }
      }
      System.out.printf("Program %d decreased by %d bytes, new size = %d\n\n", id, shrinkBytes, newSize);
    }
  }
